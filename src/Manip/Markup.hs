...
import NicLib.Tree (readIndentedGeneral) -- NicLib
...
lineCode :: ArrowXml a => Int -> DocumentWideMacro a
                   ...
                   mkelem "table" [sattr "class" "linedCode"]
                       [ selem "tbody"
                           [ selem "tr" -- each row has two columns
                               ...
                               ] | i <- [1..] -- determined by these comprehensions
                                 | t <- ts
                           ]
                       ]
               else this
...
-- | An endomorphism on an HTML document
type DocumentWideMacro a = a XmlTree XmlTree
